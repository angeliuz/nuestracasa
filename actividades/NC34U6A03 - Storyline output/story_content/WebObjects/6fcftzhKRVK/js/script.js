...
//==========================================================================================================================
//Definimos y creamos el preload de las aplicaciones (Comun para todas) 
//==========================================================================================================================

//Carga los datos necesarios desde el fichero XML del preload
//Variables para guardar los timeout
var detener = 0;
var final = 0;
...
//Extraemos los datos necesarios para el preload
//Esta variable controla si hemos cargado video o no
var videoNo = 0;
	...
	function cargarPreload()
	{
		...
		if((xmlDocPre.getElementsByTagName("video")[0] != undefined)&&(xmlDocPre.getElementsByTagName("video")[0] != null))
		{
			...
			try{
			if((xmlDocPre.getElementsByTagName("segs_skip")[0] != undefined)&&(xmlDocPre.getElementsByTagName("segs_skip")[0] != null)) segundosBoton = xmlDocPre.getElementsByTagName("segs_skip")[0].childNodes[0].nodeValue;
			else segundosBoton = "-";
			}catch(e){}
			...
			try{
			if((idioma.getElementsByTagName("saltaren")[0].childNodes[0] != undefined)&&(idioma.getElementsByTagName("saltaren")[0].childNodes[0] != null)) saltaren = idioma.getElementsByTagName("saltaren")[0].childNodes[0].nodeValue;
			else saltaren = "";
			}catch(e){}
			
			try{
			if((idioma.getElementsByTagName("segundos")[0].childNodes[0] != undefined)&&(idioma.getElementsByTagName("segundos")[0].childNodes[0] != null)) txtSegundos = idioma.getElementsByTagName("segundos")[0].childNodes[0].nodeValue;
			else txtSegundos = "";
			}catch(e){}
			
			try{
			if((idioma.getElementsByTagName("saltar")[0].childNodes[0] != undefined)&&(idioma.getElementsByTagName("saltar")[0].childNodes[0] != null)) saltar = idioma.getElementsByTagName("saltar")[0].childNodes[0].nodeValue;
			else saltar = "";
			}catch(e){}
			
			try{
			if((xmlDocPre.getElementsByTagName("hide_lbl")[0] != undefined)&&(xmlDocPre.getElementsByTagName("hide_lbl")[0] != null)) publiSi = xmlDocPre.getElementsByTagName("hide_lbl")[0].childNodes[0].nodeValue;
			else publiSi = "";
			}catch(e){}
			
			try{
			if((xmlDocPre.getElementsByTagName("alt_lbl")[0] != undefined)&&(xmlDocPre.getElementsByTagName("alt_lbl")[0] != null)) bannerPubli = xmlDocPre.getElementsByTagName("alt_lbl")[0].childNodes[0].nodeValue;
			else bannerPubli = "";
			}catch(e){}
		...
		{
			videoNo = 1;
			$('#enlacePubli').attr("href","http://www.adrformacion.com");
...
//Creamos el preload, cargamos el video o el gif de Educaplay y la publicidad en el banner
//Controlamos si se oculta el banner de publi al cargar la aplicacion
var ocultaBanner = 0;
	
	function crearPreload()
	{	
		//Nos aseguramos de que el video esta en el servidor de Educaplay
		var comprueba = videoPre.split(".educaplay.com/pub").join("");
		
		//Si el video pertenece a Educaplay, y además la pantalla es mayor de 600 pixeles, cargamos el video, sino dejamos el gif de Educaplay
		...
		//Si hay publicidad, comprobamos que dicha publicidad este en el servidor de Educaplay, y si es así, lo caragamos en el banner de publicidad
		if(publiSi == "")
		{
			if(bannerPubli != "")
			{
			...
			{
				$('#enlacePubli').attr("href","http://www.adrformacion.com");
                $('#enlacePubliVideo').attr("href","http://www.adrformacion.com");
			}
		}
		else
		{
		...
		//Lanzamos un timeout para detener el proload pasado el tiempo establecido
		detener = setTimeout(detenerPreload,segundosDuracion*1000);
	}
    
//En caso de que el video de error, no ponemos la publi
...
//Vamos actualizando el tiempo que falta para poder saltar la publicidad
...
//Activamos el botón para saltar la publi

	function activarBoton()
	{
		$('#btnSaltar').removeClass('disable');
		$('#btnSaltar').click(function(e){e.preventDefault();videoNo = 1;detenerPreload();});
		$('#tiempoFalta').html('');
	}

//Detenemos el anuncio al terminar el tiempo, o al parar el preload, y cargamos la pantalla previa a la actividad
//Esta variable detecta si el video a terminado o no
var acabado = 0;
var contadorF = 0;
	...
	function detenerPreload()
	{
		...
		{
			final = setTimeout(detenerPreload,500);
		}
...
//Ejecutamos lo siguiente al cargar la página 
	...
	window.onload = function(){
		...
		{
			//Establecemos si queremos preload y publi o no
			var preload = 0;
		}
		else
		{
		...
		//Establecemos si queremos preload de Educaplay o no
		var preloadEducaplay = 0;
		var tiempoPreloadEducaplay = 0;
		//Establecemos si queremos marca de Educaplay
		var marcaEducaplayD = 0;
	 	...
	 	rutaRecursos="/actividades/"+$(document).data("idActividad")+"/";rutaRecursos="";
        ...
        if($(document).data("publi") == 0)
		{
	 	...
	 	//Filtramos la publicidad para la actividad de Navidad de ADR
		...
		if(preload == 1)
		{
            //No hacemos nada, lo hacemos en el fichero de publicidad de video
            //cargarDatosPreload();
            $('#bannerPubli').show();
            $('#banner').show();
            
            //Si es Videoquiz si, porque no cargamos publi de vídeo
		...
		else
		{
			...
			{
				//Cargamos el el preload de Educaplay y lo quitamos despues de los segundos establecidos
...
//Inicializamos la pantalla principal y la ayuda
	...
	function inicializarPantallaInicial()
	{
		...
		$('#btnComenzarInicio a').click(function(e){e.preventDefault();comenzar();$('.logoPersonalizado').hide();$('.franjaPersonalizada').hide();});
...
//Inicializamos la recarga de la aplicación
    ...
    else
    {
		 		...
		 		$("#btnReiniciar").click(function(e){e.preventDefault();location.reload();});
		 		$("#tooltipR").html(txtReiniciar);
		 		$("#enlaceReiniciar").click(function(e){e.preventDefault();location.reload();});
		}
 	}
...
//Obtenemos el idGrupo de la actividad
    ...
    function contador()
    {
        ...
        if(actual.length > 5)
        {
            ...
            if((horas==0)&&(minutos==0)&&(segundos==0))
            {
            	...
            	if(tipo_actividad == "Test") { $('#btnFinalizar').unbind("click"); cargarProgreso(); corregir("final");}
    ...
    function cargarPantallaFinal(tipoAlerta)
    {
		...
		else
		{
			if ((tipoAlerta == "tiempo")||(tipoAlerta == "intentos"))
			{
                ...
                {
                    if(tipoAlerta == "tiempo")compruebaTexto("finTiempo");
                    else compruebaTexto("finIntentos");
...
//Funciones para el registro en base de datos de los datos obtenidos
	...
	function finalizar(){
		...
		if (registro.getElementsByTagName("tipo")[0].childNodes[0].nodeValue == "scorm"){
			...
			_finalizarScript();
		}else if (registro.getElementsByTagName("tipo")[0].childNodes[0].nodeValue == "JS"){
			_finalizarJS();
		}
		try{
			parent.finActividad();
	...
	function _finalizarJS(){
		...
		var numParametros = parametrosReg.getElementsByTagName("parametro").length;
		for(i=0;i<numParametros;i++){
			cadenaParametros += "'"+ _getValor(parametrosReg.getElementsByTagName("parametro")[i].getElementsByTagName("valor")[0].childNodes[0].nodeValue) +"'";
			if(i < numParametros-1) cadenaParametros += ",";
		}
		
		nombreFuncion=registro.getElementsByTagName("url")[0].childNodes[0].nodeValue;
		
		if($("#idColeccion").length){
			registroURL+='&idColeccion='+$("#idColeccion").html();
		}
		
		eval(nombreFuncion + '(' + cadenaParametros + ')');
	...
	function _finalizarScript(){
        ...
        $.ajaxSetup({
            ...
            {
                xhr.setRequestHeader("eduToken",codigoM);     
            }
        });
...
//Creamos los elementos necesarios para la aplicacion de Relacionar Secuencial
	...
	{	
        ...
        var arrayGrad = ['172.8','159','145.2','131.4','117.6','103.8','90','76.2','62.4','48.6','34.8','21','7.2','-7.2','-21','-34.8','-48.6','-62.4','-76.2','-90','-103.8','-117.6','-131.4','-145.2','-159','-172.8'];
    ...
    $(window).load(function(e) {
        ...
        $("#siguiente").click(function(){
			if(parseInt(elementoActivo)+1 == 26) activaElemento(0);
...
//Gestionamos el elemento activo
    ...
    {		if(reproduciendo) {
	        ...
	        $("#answer").focus().val("");
	        var contControl = 0;
	        while(($("#circle"+elem).hasClass('LetterOK') || $("#circle"+elem).hasClass('LetterError') || $("#circle"+elem).hasClass('turnOff')) && (contControl < 26)){
	            contControl++;
	            elem++;
	            if(elem == 26) elem = 0;
	        }
	        if(contControl!=26){
	            ...
	            case 'IMAGEN':
								var errorImagen = 0;
									...
									$("#questionTxtI").show();
									$('#questionTxtI').html("<img src='/img_html5/loader.gif'>");
									var img = new Image();
									img.src = rutaPista;
									img.id = 'imagenPista';
                ...
                for (var key in palabrasCorrectas) {
                    responderEscritaXapi(key,elementosFinal[key],palabrasCorrectas[key],1) 
                }
                for (var key in palabrasErroneas) {
                    responderEscritaXapi(key,elementosFinal[key],palabrasErroneas[key],0) 
                }
